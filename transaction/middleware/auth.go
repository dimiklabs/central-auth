...
//  1. Validate transaction_token (service-specific, short-lived, SameSite=Strict).
...
func RequireAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Tier 2: central identity token → exchange for transaction_token.
		central, err := c.Cookie("central_auth")
		if err != nil {
			slog.Warn("access_denied", slog.String("service", "transaction"), slog.String("reason", "no_token"), slog.String("ip", c.ClientIP()), slog.Any("request_id", rid))
		...
		if err != nil {
			slog.Warn("access_denied", slog.String("service", "transaction"), slog.String("reason", "invalid_central_token"), slog.String("ip", c.ClientIP()), slog.Any("request_id", rid))
			c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
			c.Abort()
			return
		}
		...
		if err != nil {
			slog.Error("token_issue_failed", slog.String("service", "transaction"), slog.String("user_id", cc.Subject), slog.Any("request_id", rid))
			c.JSON(http.StatusInternalServerError, gin.H{"error": "could not issue service token"})
			c.Abort()
			return
		}

		// SameSite=Strict: transaction_token is only sent by same-site fetch calls.
		// No Domain attribute → host-only: only sent to api.transaction.centralauth.local.
